...
const Work = () => {
  ...
  const [size,setSize] = useState(window.innerHeight/1.49)
  ...
  useEffect(()=>{
    ...
    setSize((orientation === 'landscape')?winSize.height/1.49:winSize.width/1.49);
  })
  ...
  var renderList = ({item, index}) => (
    <View style={styles.listelement}>
      {setLastIndex(index)}
      <Workbox size={size} title={item.title} description={item.description} languages={item.languages} link={item.link} preview={item.preview}/>
    </View>
  )
    ...
    <View style={styles.container}>
      ...
      onPress={() => {
        if(flatIndex>0){
          ...
          <Image source={arrow} style={[styles.arrowImg, {transform: [{rotate: (orientation === 'landscape')?'180deg':'-90deg'}]}]}/>
      </TouchableOpacity>
      ...
      onPress={() => {
        if(flatIndex < lastIndex-1){
          ...
          <Image source={arrow} style={[styles.arrowImg, {transform: [{rotate: (orientation === 'landscape')?'0deg':'90deg'}]}]}/>
        </TouchableOpacity>
    </View>
